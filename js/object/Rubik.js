...
export default class Rubik {
  ...
  model(type) {
    ...
    // 外层透明正方体
    // 为了简化判断滑动平面逻辑需要在所有小方块外层加一层透明的大方块
    ...
    // 在初始化基础材质时，如果想要它透明只需要指定其参数opacity为0即可；
  ...
  getDirection(sub, normalize) {
    // 计算的sub滑动向量是基于世界坐标系的，而刚刚定义的魔方的六种转动方向向量是基于魔方坐标系的，
    // 直接进行运算肯定不能得到我们想要的结果，需要把sub滑动向量转换到魔方坐标系或者把魔方的六种转动方向向量转换到世界坐标系。
    this.updateCurLocalAxisInWorld();
    let direction;
    // 判断差向量和x、y、z轴的夹角
    // 六个方向向量中谁和滑动方向向量的夹角最小谁就是此次滑动的转动方向。
